...
#if defined(NDEBUG)
#error "Crca cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ucrca) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {    // premine
      ...
      nSubsidy = 0 * COIN;
    } else if (nHeight > 245 && nHeight <= 1000000) {
      nSubsidy = 10 * COIN;
    } else {
      nSubsidy = 10 * COIN;
    }
