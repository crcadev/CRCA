...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "crca.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "crcad.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 12888, 11555));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in Crca/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in Crca/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, proxy, crca, (obfuscation, swiftx, masternode, mnpayments)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in Crca/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Crca specific functionality (Masternodes, Obfuscation, SwiftX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:12888"));
    ...
    strUsage += HelpMessageOpt("-anonymizecrcaamount=<n>", strprintf(_("Keep N Crca anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2019 The Bitcoin Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2014-2019 The Dash Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2015-2019 The PIVX Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2017-2019 The XDNA Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(_("Copyright (C) 2019 The Crca Core Developers")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Crca is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Crca Core is shutting down."));
...
#endif
    // Make sure only a single Crca process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Crca Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Crca Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Crca Core to complete") << "\n";
    ...
    nAnonymizePrjAmount = GetArg("-anonymizecrcaamount", 0);
    ...
    LogPrintf("Anonymize Crca Amount %d\n", nAnonymizePrjAmount);
    ...
    /* Denominations
       ...
       For example:
       1Crca+1000 == (.1Crca+100)*10
       10Crca+10000 == (1Crca+1000)*10
    */
